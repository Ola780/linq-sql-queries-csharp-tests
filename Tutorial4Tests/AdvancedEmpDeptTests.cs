...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnFirstTwoHiredEmployees()
    {
        ...
        var firstTwo = emps
            .OrderBy(emp => emp.HireDate).Take(2).ToList();
    ...
    public void ShouldReturnDistinctJobTitles()
    {
        ...
        var jobs = emps
            .Select(emp => emp.Job).Distinct().ToList();
    ...
    public void ShouldFindAnyWithCommissionOver400()
    {
        ...
        var result = emps.Where(emp => emp.Comm != null && emp.Comm > 400).ToList();
    ...
    public void ShouldReturnTotalIncomeIncludingCommission()
    {
        ...
        var result = emps
            .Select(emp => new
            { emp.EName, dochod = emp.Sal + (emp.Comm ?? 0)
            }).ToList();
         Assert.Contains(result, r => r.EName == "ALLEN" && r.dochod == 1900);
    }
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            .Join(depts,
                ...
                dept => dept.DeptNo,
                (employee, dept) => new { Emp = employee, Dept = dept })                       
            .SelectMany(
                para => grades,
                (para, salgrade) => new
                {
                    para.Emp.EName,para.Dept.DName, salgrade.Grade, para.Emp.Sal, salgrade.Losal, salgrade.Hisal
                })
            .Where(rekord => rekord.Sal >= rekord.Losal && rekord.Sal <= rekord.Hisal)
            .Select(pozostale => new { pozostale.EName, pozostale.DName, pozostale.Grade }).ToList();
